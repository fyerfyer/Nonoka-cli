...
def _setup(
  ...
  command = (
    ...
    f"printf '%s' {_encoded(swe_profile(model, temperature, run_timeout))} | base64 -d > /root/.config/opencode/opencode.json; "
    "/opt/nonoka-runtime/venv/bin/python -Es -c 'import nonoka, nonoka_cli'; "
    "/opt/miniconda3/bin/conda run -n testbed python -c 'import sys; print(sys.executable)'; "
    "rg --version; "
  ...
  try:
    ...
    env.update(
      {
        ...
        "NONOKA_PROTECTED_WORKSPACE_PATHS": os.pathsep.join(protected_test_paths(instance)),
      }
    )
    command = build_testbed_exec_command(
      ...
      command=[
        ...
        "--evidence-log",
        "/logs/agent/run-evidence.ndjson",
        "--artifact-dir",
        "/logs/artifacts/agent",
        "--allow-scorable-budget-exit",
        "--",
        "/opt/nonoka-runtime/opencode",
