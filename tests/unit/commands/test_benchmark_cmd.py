...
def test_smoke_writes_manifest_and_captures_opencode_output(tmp_path: Path, monkeypatch):
  ...
  monkeypatch.setattr("nonoka_cli.commands.benchmark_cmd.subprocess.run", lambda *a, **k: completed)
...
def test_terminal_bench_can_install_one_pinned_task_without_scoring(tmp_path: Path, monkeypatch):
  ...
  monkeypatch.setattr(
    ...
    lambda command, **_: (
      calls.append(command) or mock.MagicMock(returncode=0, stdout="", stderr="")
    ),
  )
